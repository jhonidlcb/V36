...
async function verifyRecaptcha(token: string): Promise<boolean> {
  ...
  try {
    ...
    // We consider it valid if the score is above 0.5
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      if (recaptchaToken) {
        ...
        if (!isValidRecaptcha) {
          console.log('❌ Login attempt blocked by reCAPTCHA');
  ...
  app.get("/api/client/stage-invoices/:stageId/download", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      doc.fontSize(9).fillColor('#1e293b').font('Helvetica-Bold');
      doc.text(`₲ ${amountPYG.toLocaleString('es-PY')}`, totalsX + 120, yPos + 26);

      doc.fontSize(8).fillColor('#475569').font('Helvetica');
      doc.text('VAT (Exempt):', totalsX + 12, yPos + 42);
      doc.fontSize(9).fillColor('#1e293b').font('Helvetica-Bold');
  ...
  app.get("/api/client/stage-invoices/:stageId/download-resimple", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      if (existingInvoice.length > 0 && existingInvoice[0].invoiceNumber) {
        ...
        if (existingInvoice.length > 0) {
          ...
          if (!existingInvoice[0].issueDateSnapshot || !existingInvoice[0].issueDateTimeSnapshot) {
            ...
            console.log(`📸 Adding IMMUTABLE issue date to invoice ${boletaNumber}: ${issueDateTime}`);
          }
      ...
      doc.fontSize(9).fillColor('#1e293b').font('Helvetica-Bold');
      doc.text(`₲ ${amountPYG.toLocaleString('es-PY')}`, valueColumnX, yPos + 26, { width: valueWidth, align: 'right' });
      ...
      // Posición correcta después del cuadro de totales
